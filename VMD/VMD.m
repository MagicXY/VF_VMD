...
% ---------------------
% signal  - the time domain signal (1D) to be decomposed输入信号
...
while ( uDiff > tol &&  n < N ) % not converged and below iterations limit
    ...
    % update first mode accumulator此处注意在k为1时迭代增量与k不为1时不同
    k = 1;
    sum_uk = u_hat_plus(n,:,K) + sum_uk - u_hat_plus(n,:,1);
    
    % update spectrum of first mode through Wiener filter of residuals
    % 此处注意lambda_hat符号与论文相反，看后面迭代过程
    ...
    for k=2:K
        ...
        % accumulator迭代思想，增量相等，初始值相等，结果即相等
