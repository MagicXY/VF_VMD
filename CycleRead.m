...
close all;
clc;
[S,F]=rdsamp('f:\Database\CUDB\cu01\cu01',[],(60*8+28)*250);
% B = 1/5*ones(5,1);
% out = filter(B,1,F); 
L=101;% the number of read_loop
R=zeros(1,L);
%--------------- Preparation
...
for l= 1:L
% t=S((60*0+(l-1)*5)*250+1:(60*0+(l)*5)*250,1);
f=F((60*0+(l-1)*5)*250+1:(60*0+(l)*5)*250,1);
% plot(t,f)
% plot(S,F)
...
%     plot(t,u(2,:)+u(3,:)+u(4,:));
    r_temp = corrcoef(u(2,:),u(2,:)+u(3,:)+u(4,:));
    R(l)=r_temp(1,2);
    disp(num2str(l));
%     if ~isempty(fsub)
